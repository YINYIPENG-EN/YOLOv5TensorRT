...
// Tensorrt 文档https://docs.nvidia.com/deeplearning/tensorrt/api/c_api/classnvinfer1_1_1_i_runtime.html
...
bool parse_args(int argc, char** argv, std::string& wts, std::string& engine, bool& is_p6, float& gd, float& gw,std::string& img_dir, std::string& video_path)
{
    if (argc < 4) return false;
	//./yolov5.exe -s yolov5s.wts yolov5s.engine s  生成engine文件
	if (std::string(argv[1]) == "-s" && (argc == 5 || argc == 7)) {  //模型类型
	  ...
	  //检测图像 ./yolov5.exe -d yolov5s.engine -img ./pictures
    } 
	else if (std::string(argv[1]) == "-d" && argc == 5)
	{	

        engine = std::string(argv[2]); //engine路径

		if (std::string(argv[3]) == "-img")
		{
			img_dir = std::string(argv[4]); //图像路径
		}
		...
		if (std::string(argv[3]) == "-video")
		{
			...
			video_path = std::string(argv[4]); //图像路径
		}
...
int main(int argc, char** argv) {
    cudaSetDevice(DEVICE); //选择cuda
    std::string wts_name = ""; //wts文件名字
    std::string engine_name = ""; //engine_name名字
    bool is_p6 = false;
	
    float gd = 0.0f, gw = 0.0f;
    std::string img_dir;  //图像路径
	std::string video_path;//视频路径
	std::string classes_path = "./coco_classes.txt"; //类文件路径
	std::vector<std::string> classes_name = get_classes(classes_path);
	//传参
    if (!parse_args(argc, argv, wts_name, engine_name, is_p6, gd, gw,img_dir, video_path)) {
		//cerr是输出错误流，写到cerr中的数据是不缓冲的，直接输出到屏幕
    ...
    if (!wts_name.empty()) 
	{
		...
		//reinterpret_cast(必须是一个指针)强制类型转换  即把modelStream->data()强转成char类型
	...
	//读取图像文件，img_dir = ./pictures
	std::vector<std::string> file_names;
	if (!img_dir.empty())
	{
		//string.c_str()可以将string类型转为const char* 类型
	...
	// prepare input data ---------------------------
	static float data[BATCH_SIZE * 3 * INPUT_H * INPUT_W]; //这里的3是3通道
	...
	{	
		...
		cv::VideoCapture cap(atoi(video_path.c_str()));
		cv::VideoWriter outvideo;
		int fource = cv::VideoWriter::fourcc('X','V','I','D');
		cv::Size S = cv::Size((int)cap.get(cv::CAP_PROP_FRAME_WIDTH), (int)cap.get(cv::CAP_PROP_FRAME_HEIGHT));
		outvideo.open("test.avi", fource, 25.0, S, true);
		cv::Mat frame;
		if (!cap.isOpened())
		{
		...
		while (true)
		{
			...
			//if (frame.empty()) continue; //判断图像是否为空
			cv::Mat pr_img = preprocess_img(frame, INPUT_W, INPUT_H); // letterbox BGR to RGB  图像预处理 shape=640*640
			int b = 0, i = 0;
			int fcount = 1;
			...
			for (int b = 0; b < fcount; b++)
			{
				...
				for (size_t j = 0; j < res.size(); j++) {
					...
					cv::putText(frame, classes_name[(int)res[j].class_id], cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0xFF, 0xFF, 0xFF), 2);
				}
			}
		...
		cv::destroyWindow("YOLOV5 Detection");
		cap.release();
	}
	...
	{
		int fcount = 0;
		for (int f = 0; f < (int)file_names.size(); f++) {  //循环读取图像
			fcount++;
			if (fcount < BATCH_SIZE && f + 1 != (int)file_names.size()) continue;
			for (int b = 0; b < fcount; b++) { // b就是batchsize了
				cv::Mat img = cv::imread(img_dir + "/" + file_names[f - fcount + 1 + b]);
				if (img.empty()) continue; //判断图像是否为空
				cv::Mat pr_img = preprocess_img(img, INPUT_W, INPUT_H); // letterbox BGR to RGB  图像预处理
				int i = 0;
				for (int row = 0; row < INPUT_H; ++row) { //遍历图像像素
					uchar* uc_pixel = pr_img.data + row * pr_img.step;
					for (int col = 0; col < INPUT_W; ++col) {
						data[b * 3 * INPUT_H * INPUT_W + i] = (float)uc_pixel[2] / 255.0;
						data[b * 3 * INPUT_H * INPUT_W + i + INPUT_H * INPUT_W] = (float)uc_pixel[1] / 255.0;
			...
			for (int b = 0; b < fcount; b++) {
				...
				for (size_t j = 0; j < res.size(); j++) {
					...
					cv::putText(img, classes_name[(int)res[j].class_id], cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0xFF, 0xFF, 0xFF), 2);
				}
